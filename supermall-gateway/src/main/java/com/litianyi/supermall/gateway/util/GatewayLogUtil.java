package com.litianyi.supermall.gateway.util;

import com.litianyi.supermall.gateway.context.BaseContextHandler;
import com.litianyi.supermall.gateway.decorator.RecorderServerHttpResponseDecorator;
...
public class GatewayLogUtil {
    ...
    public static Mono<Void> recorderResponse(ServerWebExchange exchange) {
        ...
        logBuffer.append("响应时间：")
                .append(Duration.between((Instant) BaseContextHandler.getContextMap().get("beginTime"), Instant.now()).toMillis())
                .append("ms").append('\n');
